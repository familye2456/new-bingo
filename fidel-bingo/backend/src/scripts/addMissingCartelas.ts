...
async function addMissingCartelas() {
  ...
  for (const card of missingCards) {
    const numbers: number[] = card.grid
      .flat()
      .map((cell) => (typeof cell === 'string' ? 0 : cell));

    // Upsert cartela: find by card_number, create if missing
    let cartela = await cartelaRepo.findOne({ where: { cardNumber: card.board_number } });

    if (!cartela) {
      const patternMask: boolean[] = Array(25).fill(false);
      patternMask[12] = true;
      cartela = cartelaRepo.create({
      ...
      console.log(`  Created cartela #${card.board_number}`);
    } else {
      console.log(`  Cartela #${card.board_number} already exists (id: ${cartela.id})`);
    }
    ...
    if (existing) {
      console.log(`  Cartela #${card.board_number} already assigned to ${TARGET_USERNAME} — skipping`);
      skipped++;
      continue;
    }
    ...
    console.log(`  Assigned cartela #${card.board_number} to ${TARGET_USERNAME}`);
    assigned++;
  }
