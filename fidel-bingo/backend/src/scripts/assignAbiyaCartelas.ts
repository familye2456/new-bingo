...
const TARGET_USERNAME = 'abiya';

const cartelaPath = path.join(__dirname, '../../assets/abiyacartela.js');
// eslint-disable-next-line @typescript-eslint/no-var-requires
const { default: cartelas } = require(cartelaPath) as { default: Record<number, (number | string)[][]> };
...
async function run() {
  ...
  // 3. Insert cartelas from abiyacartela.js directly into user_cartelas
  const cardIds = Object.keys(cartelas).map(Number).sort((a, b) => a - b);
  console.log(`\nInserting ${cardIds.length} cartelas from abiyacartela.js into user_cartelas...\n`);

  let inserted = 0;
  for (const cardId of cardIds) {
    const grid = cartelas[cardId];
    const numbers: number[] = grid.flat().map((cell) =>
      typeof cell === 'string' ? 0 : (cell ?? 0)
    );
